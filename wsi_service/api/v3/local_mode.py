...
def add_routes_local_mode(app, settings):
    ...
    async def _(slide_id: str = IdQuery, payload=api_integration.global_depends()):
        ...
        await api_integration.allow_access_slide(
            auth_payload=payload, slide_id=slide_id, manager=None, plugin=None
        )
    ...
    @app.get(
        "/refresh_local_mapper", tags=["Additional Routes (Standalone WSI Service)"]
    )
    async def _(payload=api_integration.global_depends()):
        """
        ...
        localmapper.refresh()
        return JSONResponse(
            {"detail": "Local mapper has been refreshed."}, status_code=200
        )
