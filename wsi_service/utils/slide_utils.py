...
def get_original_levels(level_count, level_dimensions, level_downsamples):
    ...
    for level in range(level_count):
        levels.append(
            SlideLevel(
                extent=SlideExtent(
                    x=level_dimensions[level][0], y=level_dimensions[level][1], z=1
                ),
...
def get_rgb_channel_list():
    ...
    channels.append(
        SlideChannel(id=1, name="Green", color=SlideColor(r=0, g=255, b=0, a=0))
    )
    channels.append(
        SlideChannel(id=2, name="Blue", color=SlideColor(r=0, g=0, b=255, a=0))
    )
    return channels
