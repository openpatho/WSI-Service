...
from wsi_service.utils.image_utils import save_rgb_image
...
async def safe_get_slide_for_query(slide_manager, path, plugin):
    try:
        return await slide_manager.get_slide_info(
            path, slide_info_model=SlideInfo, plugin=plugin
        )
    except Exception as e:
...
def batch_safe_make_response(
    slides, image_regions, image_format, image_quality, image_channels=None
):
...
async def batch_safe_get_region(
    slide,
    slide_info,
    level,
    start_x,
...
async def batch_safe_get_tile(
    slide, slide_info, level, tile_x, tile_y, image_channels, vp_color, z
):
    try:
        validate_image_level(slide_info, level)
        validate_image_z(slide_info, z)
